...
export default function Banda(){
  ...
  return <main className="public-site inner-site" style={{"--accent":content.brand.accentColor} as React.CSSProperties}><PublicHeader content={content}/><PageHero kicker="01 / A BANDA" title={content.about.heading}/><section className="about-route"><p>{content.about.text}</p></section><section className="member-lineup" aria-label="Integrantes da Valete">{content.members.map((m,i)=><Dialog key={m.id}><DialogTrigger asChild><button className="member-card" aria-label={`Conheça ${m.name}`}><div className="member-card-photo">{m.imageUrl?<img src={m.imageUrl} alt={m.name}/>:<><strong>{m.name[0]}</strong><span>FOTO EM BREVE</span></>}<div className="member-symbol"><MemberSymbol id={m.id} role={m.role} iconUrl={m.iconUrl}/></div></div><div className="member-card-caption"><small>0{i+1} · {m.role}</small><h2>{m.name}</h2><span>CONHEÇA A HISTÓRIA →</span></div></button></DialogTrigger><DialogContent className="member-modal"><div className="member-modal-photo">{m.imageUrl?<img src={m.imageUrl} alt={m.name}/>:<strong>{m.name[0]}</strong>}<div className="member-symbol"><MemberSymbol id={m.id} role={m.role} iconUrl={m.iconUrl}/></div></div><div className="member-modal-copy"><DialogHeader><small>{m.role}</small><DialogTitle>{m.name}</DialogTitle><DialogDescription>A história de {m.name} na música e na Valete.</DialogDescription></DialogHeader><p>{m.bio}</p>{m.instagramUrl&&<a className="member-instagram" href={m.instagramUrl} target="_blank" rel="noreferrer"><Camera aria-hidden="true"/> INSTAGRAM DE {m.name} <span>↗</span></a>}</div></DialogContent></Dialog>)}</section><PublicFooter content={content}/></main>;
}
